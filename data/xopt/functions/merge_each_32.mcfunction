...
execute at @s run data merge entity @e[limit=1,sort=nearest,type=minecraft:experience_orb,nbt={Value:32s},distance=..0.5] {Value:-2s}

# if a friend was detected, update the original orb to the merged value
execute if entity @e[type=minecraft:experience_orb,nbt={Value:-2s}] run data merge entity @s {Value:64s}

# and kill the friend
kill @e[type=minecraft:experience_orb,nbt={Value:-2s}]

# reset now if the value was not merged
data merge entity @s[nbt={Value:-1s}] {Value:32s}
